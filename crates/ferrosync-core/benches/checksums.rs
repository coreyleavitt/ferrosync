    ...
    for &size in sizes {
        ...
        for &(name, alg) in &algorithms {
            group.bench_with_input(
                ...
                |b, data| {
                    b.iter(|| {
                        ferrosync_core::delta::checksum::checksum2(data, 42, alg, true)
                    });
                },
            );
        }
    ...
    for &size in sizes {
        ...
        for &(name, alg) in &algorithms {
            group.bench_with_input(
                ...
                |b, data| {
                    b.iter(|| {
                        ferrosync_core::delta::checksum::file_checksum(data, 42, alg)
                    });
                },
            );
        }
...
fn bench_rolling_checksum(c: &mut Criterion) {
    ...
    for &size in sizes {
        ...
        group.bench_with_input(
            BenchmarkId::new("checksum1", size),
            &data,
            |b, data| {
                b.iter(|| {
        ...
        group.bench_with_input(
            BenchmarkId::new("rolling_slide", size),
            &data,
            |b, data| {
                b.iter(|| {
                    let block_len = 700;
...
criterion_group!(benches, bench_checksum2, bench_file_checksum, bench_rolling_checksum);
