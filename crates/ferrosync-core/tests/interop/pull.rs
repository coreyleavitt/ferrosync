...
async fn test_interop_pull_single_file() {
    ...
    let meta = std::fs::metadata(env.dst().join("pull.txt")).unwrap();
...
async fn test_interop_pull_directory_recursive() {
    ...
    assert!(env.dst().join("sub").is_dir(), "sub/ should be a directory");
...
async fn test_interop_pull_archive_mode() {
    ...
    let src_mtime = std::fs::metadata(env.src().join("archive.txt"))
        .unwrap()
        .modified()
        .unwrap()
        ...
        .unwrap()
        .as_secs() as i64;
    assert_mtime(&env.dst().join("archive.txt"), src_mtime, 1);
...
async fn test_interop_pull_max_delete() {
    ...
    assert_eq!(
        std::fs::read(env.dst().join("keep.txt")).unwrap(),
        b"keep\n"
    );
...
async fn test_interop_pull_inplace() {
    ...
    let inode_before = crate::common::env::inode_of(&env.dst().join("file.txt"));
    ...
    let inode_after = crate::common::env::inode_of(&env.dst().join("file.txt"));
    assert_eq!(
...
async fn test_interop_pull_itemize() {
    ...
    let mtime = std::fs::metadata(env.dst().join("file.txt"))
        .unwrap()
        .modified()
        .unwrap()
...
async fn test_interop_pull_bwlimit() {
    ...
    let opts = ferrosync_core::options::TransferOptions::builder()
        ...
        .build();
    let remote_path = format!("{remote_dir}/");
    let result = pull_with_opts(opts, &remote_path, 60).await;
...
async fn test_interop_pull_hardlinks_basic() {
    ...
    // ssh_cmd concatenates args into one remote command, so pass as a single string.
    ssh_cmd(&[&format!(
        "echo shared_content > {remote_dir}/original.txt && \
         ln {remote_dir}/original.txt {remote_dir}/linked.txt"
    )])
    .await;
    ...
    let content_a = std::fs::read(env.dst().join("original.txt")).unwrap();
    let content_b = std::fs::read(env.dst().join("linked.txt")).unwrap();
...
async fn test_interop_pull_hardlinks_multiple_groups() {
    ...
    ssh_cmd(&[&format!(
        "echo group_a > {remote_dir}/a1.txt && \
         ln {remote_dir}/a1.txt {remote_dir}/a2.txt"
    )])
    .await;
    // Group B: two hardlinked files with different content.
    ssh_cmd(&[&format!(
        "echo group_b > {remote_dir}/b1.txt && \
         ln {remote_dir}/b1.txt {remote_dir}/b2.txt"
    )])
    .await;
    ...
    assert_hard_linked(&env.dst().join("a1.txt"), &env.dst().join("a2.txt"));
    // Within group B: hardlinked.
    assert_hard_linked(&env.dst().join("b1.txt"), &env.dst().join("b2.txt"));
    // Across groups: NOT hardlinked.
    assert_not_hard_linked(&env.dst().join("a1.txt"), &env.dst().join("b1.txt"));
